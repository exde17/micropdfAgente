...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Puedes restringirlo a ["http://tudominio.com"]
    allow_credentials=True,
    allow_methods=["*"],  
    allow_headers=["*"],  
)
...
async def generate_report(data: dict):
    ...
    Genera un reporte en PDF basado en los datos recibidos.
    """
    tipo_reporte = data.get("tipo", "").lower()

    # Si el tipo es "informe" generamos un PDF sin gráficas
    if tipo_reporte == "informe":
        pdf_binary = generate_pdf(data, include_graph=False)
    else:
        pdf_binary = generate_pdf(data, include_graph=True)
...
async def receive_data(request: Request):
    ...
    if tipo == "reporte" or tipo == "informe":
        # Pasar el JSON directamente al endpoint de generación de reportes
        return await generate_report(data)
        ...
        return {"error": "Tipo de solicitud no reconocido. Usa 'reporte', 'informe' o 'natural'."}
